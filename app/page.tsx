      ...
      <div className={`max-w-5xl mx-auto px-4 py-8 transition-all duration-500 ease-in-out ${!isSearching ? 'opacity-0 pointer-events-none scale-105 absolute inset-0' : 'opacity-100 scale-100 relative z-10'}`}>
        <header className="flex justify-between items-center mb-8 border-b border-slate-800/80 pb-4 relative">
          ...
          <div className="flex items-center gap-4">
            <a href="/upload" className="bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 px-4 py-2 rounded-xl text-xs font-semibold transition-all shadow-sm">
              + Upload
            </a>
          ...
          >
            <div>
              <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
                ...
                <button onClick={() => setMenuOpen(false)} className="text-slate-400 hover:text-white p-2 rounded-lg bg-slate-950 border border-slate-800 text-xs">
                  ✕ Close
                </button>
              </div>
              ...
              <div className="space-y-3">
                <button onClick={handleProfileClick} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 text-slate-200 text-sm font-medium transition-all text-left">
                ...
                <a href="/upload" onClick={() => setMenuOpen(false)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-950 hover:bg-slate-800/80 border border-slate-800 text-slate-200 text-sm font-medium transition-all">
                  <svg className="w-5 h-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {isAdmin && (
                  <a href="/admin" onClick={() => setMenuOpen(false)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-gradient-to-r from-blue-600/20 to-red-600/20 hover:from-blue-600/30 hover:to-red-600/30 border border-red-500/30 text-red-300 text-sm font-semibold transition-all shadow-md">
                    <svg className="w-5 h-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {loading ? (
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {filteredMaterials.map((mat) => (
              <div key={mat.id} className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-slate-800 hover:border-slate-700 transition-all group flex flex-col justify-between">
                <div>
